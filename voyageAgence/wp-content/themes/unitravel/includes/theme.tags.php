...
if ( !function_exists('unitravel_need_page_title') ) {
	function unitravel_need_page_title() {
		return !is_front_page() && apply_filters('unitravel_filter_need_page_title', true);
	}
}
...
if ( !function_exists('unitravel_show_layout') ) {
	function unitravel_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('unitravel_get_logo_image') ) {
	function unitravel_get_logo_image($type='') {
		$logo_image = '';
		if (unitravel_get_retina_multiplier(2) > 1)
			$logo_image = unitravel_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = unitravel_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('unitravel_get_header_video') ) {
	function unitravel_get_header_video() {
		$video = '';
		if (apply_filters('unitravel_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (unitravel_check_theme_option('header_video')) {
				$video = unitravel_get_theme_option('header_video');
...
if ( !function_exists('unitravel_show_post_meta') ) {
	function unitravel_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			if ( !empty($args['categories'])) {
				...
				if (!empty($cats)) {
					?>
					<span class="post_meta_item post_categories"><?php unitravel_show_layout($cats); ?></span>
					<?php
				}
			}
			// Post date
			if ( !empty($args['date'])) {
				$dt = apply_filters('unitravel_filter_get_post_date', unitravel_get_date());
				if (!empty($dt)) {
					?>
			...
			if ( !empty($args['author'])) {
				?>
				<a class="post_meta_item post_author icon-user" rel="author" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>">
			...
			// Post counters
			unitravel_show_layout(unitravel_get_post_counters($args['counters']));

			// Socials share
			if ( !empty($args['share']) ) {
				unitravel_show_share_links(array(
						'type' => 'drop',
						'caption' => esc_html__('Share', 'unitravel'),
			...
			// Edit page link
			if ( !empty($args['edit']) ) {
				edit_post_link( esc_html__( 'Edit', 'unitravel' ), '<span class="post_meta_item post_edit icon-pencil">', '</span>' );
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('unitravel_show_post_featured') ) {
	function unitravel_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => unitravel_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : unitravel_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		...
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				...
				$image = unitravel_get_post_image();
				if (!empty($image)) 
					$image = unitravel_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = unitravel_get_no_image('', !empty($args['show_no_image']));
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . unitravel_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="//schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$unitravel_attr = unitravel_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						unitravel_hovers_add_icons($args['hover']);
					}
					...
					if ( ($output = unitravel_build_slider_layout(array('thumb_size' => $thumb_size, 'controls' => 'yes', 'pagination' => 'yes'))) != '' )
						unitravel_show_layout($output);
					...
					if (!empty($image)) {
						...
						unitravel_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				// Put video under the thumb
					if ($post_format == 'video') {
						$video = unitravel_get_post_video($post_content, false);
					if (empty($video))
							$video = unitravel_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = unitravel_array_get_first($videos, false);
								}
							}
						}
					if (!empty($video)) {
						if ( $has_thumb ) {
							$video = unitravel_make_video_autoplay($video);
							?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
						}
						?><div class="post_video video_frame"><?php 
							if ( !$has_thumb ) {
								unitravel_show_layout($video);
							}
						?></div><?php
					}
				...
				// Put audio over the thumb
						...
						$audio = unitravel_get_post_audio($post_content, false);
					if (empty($audio))
							$audio = unitravel_get_post_iframe($post_content, false);
						if (empty($audio)) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', get_the_content() );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($audios) && is_array($audios)) {
									$audio = unitravel_array_get_first($audios, false);
							}
							}
						}
						if (!empty($audio)) {
						?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
							...
							$media_author = unitravel_get_theme_option('media_author', '', false, get_the_ID());
							$media_title = unitravel_get_theme_option('media_title', '', false, get_the_ID());
							if ( !empty($media_author) && !unitravel_is_inherit($media_author) ) {
								?><div class="post_audio_author"><?php unitravel_show_layout($media_author); ?></div><?php
							}
							if ( !empty($media_title) && !unitravel_is_inherit($media_title) ) {
								?><h5 class="post_audio_title"><?php unitravel_show_layout($media_title); ?></h5><?php
							}
							// Display audio
							unitravel_show_layout($audio); 
						?></div><?php
					}
				}
				}
				...
				unitravel_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				unitravel_show_layout($post_info);
...
if ( !function_exists('unitravel_get_no_image') ) {
	function unitravel_get_no_image($no_image='', $need=false) {
		static $img = '';
		if (empty($img)) {
			$img = unitravel_get_theme_option( 'no_image' );
			// Remove false in next line if you want display theme-specific image in the posts without featured image
			if ($need && empty($img)) $img = unitravel_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('unitravel_add_bg_in_post_nav') ) {
	function unitravel_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = unitravel_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), unitravel_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), unitravel_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'unitravel-main', $css );
	}
}

// Show related posts
if ( !function_exists('unitravel_show_related_posts') ) {
	function unitravel_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h3 class="section title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'unitravel');
				?></h3>
...
if ( !function_exists('unitravel_show_portfolio_posts') ) {
	function unitravel_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? unitravel_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = unitravel_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = unitravel_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the unitravel_ajax_get_posts action
if ( !function_exists( 'unitravel_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_unitravel_ajax_get_posts',			'unitravel_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_unitravel_ajax_get_posts',	'unitravel_ajax_get_posts_callback');
	function unitravel_ajax_get_posts_callback() {
		...
		if ((int) $id > 0) {
			...
			unitravel_storage_set('blog_mode', 'blog');
			unitravel_storage_set('options_meta', get_post_meta($id, 'unitravel_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => unitravel_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'unitravel');
		}
...
// Show pagination
if ( !function_exists('unitravel_show_pagination') ) {
	function unitravel_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = unitravel_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'unitravel' ),
				'next_text' => esc_html__( '>', 'unitravel' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'unitravel' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'unitravel'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'unitravel') : esc_html__('Newest posts', 'unitravel') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'unitravel') : esc_html__('Older posts', 'unitravel'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
