...
if (!function_exists('unitravel_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'unitravel_hovers_theme_setup3', 3 );
	function unitravel_hovers_theme_setup3() {

		$need_lists = is_admin() && unitravel_options_need_lists();
		
		// Add 'Buttons hover' option
		unitravel_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'unitravel'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'unitravel') ),
				"std" => 'default',
				"options" => !$need_lists ? array() :  array(
					'default'		=> esc_html__('Fade',				'unitravel'),
					'slide_left'	=> esc_html__('Slide from Left',	'unitravel'),
					'slide_right'	=> esc_html__('Slide from Right',	'unitravel'),
					'slide_top'		=> esc_html__('Slide from Top',		'unitravel'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'unitravel'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'unitravel'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'unitravel') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => !$need_lists ? array() :  array(
					'dots'	=> esc_html__('Dots',	'unitravel'),
					'icon'	=> esc_html__('Icon',	'unitravel'),
					'icons'	=> esc_html__('Icons',	'unitravel'),
					'zoom'	=> esc_html__('Zoom',	'unitravel'),
					'fade'	=> esc_html__('Fade',	'unitravel'),
					'slide'	=> esc_html__('Slide',	'unitravel'),
					'pull'	=> esc_html__('Pull',	'unitravel'),
					'border'=> esc_html__('Border',	'unitravel')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('unitravel_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'unitravel_hovers_theme_setup9', 9 );
	function unitravel_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'unitravel_hovers_frontend_scripts', 1010 );
		add_filter( 'unitravel_filter_localize_script','unitravel_hovers_localize_script' );
		add_filter( 'unitravel_filter_merge_scripts',	'unitravel_hovers_merge_scripts' );
		add_filter( 'unitravel_filter_merge_styles',	'unitravel_hovers_merge_styles' );
		add_filter( 'unitravel_filter_get_css', 		'unitravel_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'unitravel_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'unitravel_hovers_frontend_scripts', 1010 );
	function unitravel_hovers_frontend_scripts() {
		if ( unitravel_is_on(unitravel_get_theme_option('debug_mode')) && unitravel_get_file_dir('includes/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'unitravel-hovers', unitravel_get_file_url('includes/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( unitravel_is_on(unitravel_get_theme_option('debug_mode')) && unitravel_get_file_dir('includes/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'unitravel-hovers',  unitravel_get_file_url('includes/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('unitravel_hovers_merge_scripts')) {
	//Handler of the add_filter( 'unitravel_filter_merge_scripts', 'unitravel_hovers_merge_scripts' );
	function unitravel_hovers_merge_scripts($list) {
...
if (!function_exists('unitravel_hovers_merge_styles')) {
	//Handler of the add_filter( 'unitravel_filter_merge_styles', 'unitravel_hovers_merge_styles' );
	function unitravel_hovers_merge_styles($list) {
...
if (!function_exists('unitravel_hovers_localize_script')) {
	//Handler of the add_filter( 'unitravel_filter_localize_script','unitravel_hovers_localize_script' );
	function unitravel_hovers_localize_script($arr) {
		$arr['button_hover'] = unitravel_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('unitravel_hovers_add_icons') ) {
	function unitravel_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'unitravel') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink());; ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'unitravel');
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'unitravel_hovers_get_css' ) ) {
	//Handler of the add_filter( 'unitravel_filter_get_css', 'unitravel_hovers_get_css', 10, 4 );
	function unitravel_hovers_get_css($css, $colors, $fonts, $scheme='') {
