...
if ( !function_exists( 'unitravel_wp_scripts' ) ) {
	//Handler of the add_action('wp_enqueue_scripts', 'unitravel_wp_scripts', 1000);
...
if ( !function_exists( 'unitravel_localize_scripts' ) ) {
	//Handler of the add_action('wp_footer', 'unitravel_localize_scripts');
...
if ( !function_exists( 'unitravel_wp_scripts_responsive' ) ) {
	//Handler of the add_action('wp_enqueue_scripts', 'unitravel_wp_scripts_responsive', 2000);
...
if (!function_exists('unitravel_wp_head')) {
	//Handler of the add_action('wp_head',	'unitravel_wp_head', 1);
...
if ( !function_exists('unitravel_add_body_classes') ) {
	//Handler of the add_filter( 'body_class', 'unitravel_add_body_classes' );
...
if ( !function_exists( 'unitravel_wp_footer' ) ) {
	//Handler of the add_action('wp_footer', 'unitravel_wp_footer');
...
if ( !function_exists('unitravel_register_sidebars') ) {
	// Handler of the add_action('widgets_init', 'unitravel_register_sidebars');
...
if ( !function_exists( 'unitravel_importer_set_options' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_importer_options',	'unitravel_importer_set_options', 9 );
	function unitravel_importer_set_options($options=array()) {
		if (is_array($options)) {
			...
			$options['files']['default']['domain_dev'] = esc_url(unitravel_get_protocol().'://unitravel.dv.ancorathemes.com');		// Developers domain
			$options['files']['default']['domain_demo']= esc_url(unitravel_get_protocol().'://unitravel.ancorathemes.com');		// Demo-site domain
