	...
	function unitravel_customizer_css_template() {
		...
		if (count($schemes) > 0) {
			foreach ($schemes as $scheme) {
				echo '<' . esc_attr($tmpl_holder) . ' type="text/html" id="tmpl-unitravel-color-scheme-'.esc_attr($scheme).'">'
		...
		if (is_array($fonts) && count($fonts) > 0) {
			...
			echo '<'.esc_attr(trim($tmpl_holder)).' type="text/html" id="tmpl-unitravel-fonts">'
...
if ( !function_exists('unitravel_init_override_options') ) {
	add_action( 'after_setup_theme', 'unitravel_init_override_options' );
	function unitravel_init_override_options() {
		if ( is_admin() ) {
			add_action("admin_enqueue_scripts", 'unitravel_add_override_options_scripts');
			add_action('save_post',			'unitravel_save_override_options');
			add_filter('trx_addons_filter_override_options',	'unitravel_add_override_options');
		}
	}
}
	
// Load required styles and scripts for admin mode
if ( !function_exists( 'unitravel_add_override_options_scripts' ) ) {
	add_action("admin_enqueue_scripts", 'unitravel_add_override_options_scripts');
	function unitravel_add_override_options_scripts() {
		// If current screen is 'Edit Page' - load fontello
		$screen = function_exists('get_current_screen') ? get_current_screen() : false;
		if (is_object($screen) && unitravel_allow_override_options(!empty($screen->post_type) ? $screen->post_type : $screen->id)) {
			wp_enqueue_style( 'fontello-style',  unitravel_get_file_url('css/fontello/fontello-embedded.css') );
			wp_enqueue_script('jquery-ui-tabs', false, array('jquery', 'jquery-ui'), null, true);
			wp_enqueue_script( 'unitravel-override-options', unitravel_get_file_url('theme-options/theme.override-options.js'), array('jquery'), null, true );
			wp_localize_script( 'unitravel-override-options', 'unitravel_dependencies', unitravel_get_theme_dependencies() );
		}
	}
}
...
if (!function_exists('unitravel_allow_override_options')) {
	function unitravel_allow_override_options($post_type) {
		return apply_filters('unitravel_filter_allow_override_options', in_array($post_type, array('page', 'post')), $post_type);
	}
}

// Add override options
if (!function_exists('unitravel_add_override_options')) {
	function unitravel_add_override_options($boxes = array()) {
		global $post_type;
		if (unitravel_allow_override_options($post_type)) {
			$boxes[] = array('id' => sprintf('unitravel_override_options_%s', $post_type),
				'title' =>  esc_html__('Theme Options', 'unitravel'),
				'callback' => 'unitravel_show_override_options',
				'page' => $post_type,
				'context' => 'advanced',
				'priority' => 'default'
			);
...
if (!function_exists('unitravel_show_override_options')) {
	function unitravel_show_override_options() {
		...
		if (unitravel_allow_override_options($post_type)) {
			...
			foreach ($UNITRAVEL_STORAGE['options'] as $k=>$v) {
				...
				$v['val'] = isset($meta[$k]) ? $meta[$k] : 'inherit';
				$tabs_content[$v['override']['section']] .= unitravel_show_override_options_field($k, $v);
			}
			if (count($tabs_titles) > 0) {
				?>
				<div class="unitravel_override_options">
					...
					<div id="unitravel_override_options_tabs">
						<ul><?php
							$cnt = 0;
							foreach ($tabs_titles as $k=>$v) {
								$cnt++;
								?><li><a href="#unitravel_override_options_<?php echo esc_attr($cnt); ?>"><?php echo esc_html($v); ?></a></li><?php
							}
						?></ul>
						<?php
							$cnt = 0;
							foreach ($tabs_content as $k=>$v) {
								$cnt++;
								?>
								<div id="unitravel_override_options_<?php echo esc_attr($cnt); ?>" class="unitravel_override_options_section">
...
if ( !function_exists('unitravel_show_override_options_field') ) {
	function unitravel_show_override_options_field($name, $field) {
		if ($field['type'] == 'hidden') return '';
		$inherit_state = unitravel_is_inherit($field['val']);
		$output = '<div class="unitravel_override_options_item unitravel_override_options_item_'.esc_attr($field['type']).' unitravel_override_options_inherit_'.($inherit_state ? 'on' : 'off' ).'">'
						. '<h4 class="unitravel_override_options_item_title">'
							. esc_html($field['title'])
							. '<span class="unitravel_override_options_inherit_lock" id="unitravel_override_options_inherit_'.esc_attr($name).'"></span>'
						. '</h4>'
						. '<div class="unitravel_override_options_item_data">'
							. '<div class="unitravel_override_options_item_field" data-param="'.esc_attr($name).'"'.(!empty($field['linked']) ? ' data-linked="'.esc_attr($field['linked']).'"' : '').'>';
		if ($field['type']=='checkbox') {
			$output .= '<label class="unitravel_override_options_item_label">'
						. '<input type="checkbox" name="unitravel_override_options_field_'.esc_attr($name).'" value="1"'.($field['val']==1 ? ' checked="checked"' : '').' />'
						. esc_html($field['title'])
					. '</label>';
		} else if ($field['type']=='switch' || $field['type']=='radio') {
			foreach ($field['options'] as $k=>$v) {
				$output .= '<label class="unitravel_override_options_item_label">'
							. '<input type="radio" name="unitravel_override_options_field_'.esc_attr($name).'" value="'.esc_attr($k).'"'.($field['val']==$k ? ' checked="checked"' : '').' />'
							. esc_html($v)
						. '</label>';
			}
		} else if ($field['type']=='text') {
			$output .= '<input type="text" name="unitravel_override_options_field_'.esc_attr($name).'" value="'.esc_attr(unitravel_is_inherit($field['val']) ? '' : $field['val']).'" />';
		} else if ($field['type']=='textarea') {
			$output .= '<textarea name="unitravel_override_options_field_'.esc_attr($name).'">'.esc_html(unitravel_is_inherit($field['val']) ? '' : $field['val']).'</textarea>';
		} else if ($field['type']=='select') {
			$output .= '<select size="1" name="unitravel_override_options_field_'.esc_attr($name).'">';
			foreach ($field['options'] as $k=>$v) {
				$output .= '<option value="'.esc_attr($k).'"'.($field['val']==$k ? ' selected="selected"' : '').'>'.esc_html($v).'</option>';
			}
			$output .= '</select>';
		} else if (in_array($field['type'], array('image', 'media', 'video', 'audio'))) {
			$output .= '<input type="text" id="unitravel_override_options_field_'.esc_attr($name).'" name="unitravel_override_options_field_'.esc_attr($name).'" value="'.esc_attr(unitravel_is_inherit($field['val']) ? '' : $field['val']).'" />'
					. unitravel_show_custom_field('unitravel_override_options_field_'.esc_attr($name).'_button', array(
														'type'				=> 'mediamanager',
														'data_type'			=> $field['type'],
														'linked_field_id'	=> 'unitravel_override_options_field_'.esc_attr($name)),
														null)
					. '<div class="unitravel_override_options_field_preview">'
						. (unitravel_is_inherit($field['val']) ? '' : ($field['val'] && $field['type']=='image' ? '<img src="' . esc_url($field['val']) . '" alt="'.esc_attr__('image', 'unitravel').'">' : basename($field['val'])))
					. '</div>';
		}
		$output .= '<div class="unitravel_override_options_inherit_cover'.(!$inherit_state ? ' unitravel_hidden' : '').'">'
							. '<span class="unitravel_override_options_inherit_label">' . esc_html__('Inherit', 'unitravel') . '</span>'
							. '<input type="hidden" name="unitravel_override_options_inherit_'.esc_attr($name).'" value="'.esc_attr($inherit_state ? 'inherit' : '').'" />'
						. '</div>'
					. '</div>'
					. '<div class="unitravel_override_options_item_description">'
...
if (!function_exists('unitravel_save_override_options')) {
	//Handler of the add_action('save_post', 'unitravel_save_override_options');
	function unitravel_save_override_options($post_id) {

		// verify nonce
		if ( !wp_verify_nonce( unitravel_get_value_gp('override_options_post_nonce'), admin_url() ) )
			return $post_id;
		...
		foreach ($options as $k=>$v) {
			...
			if (!empty($_POST['unitravel_override_options_inherit_' . $k])) continue;
			// Get option value from POST
			$meta[$k] = isset($_POST['unitravel_override_options_field_' . $k])
							? sanitize_text_field($_POST['unitravel_override_options_field_' . $k])
							: ($v['type']=='checkbox' ? 0 : '');
		}
